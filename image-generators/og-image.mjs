...
const OUT = `${LP}/public/assets/og-image.jpg`;
...
// Fontes embutidas em base64 (file:// quebra por causa do "ç" em Lançar)
...
// Wordmark REAL (Logo.astro), fill via currentColor
const WORDMARK = `<svg viewBox="0 0 290 50" height="40" fill="${T.ink}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Notifiquei">
...
const html = `<!doctype html><html><head><meta charset="utf-8"><style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  .og{position:relative;width:1200px;height:630px;overflow:hidden;background:${T.paper};font-family:'Jakarta',sans-serif}
  .glow{position:absolute;top:-200px;left:-160px;width:620px;height:620px;border-radius:50%;background:radial-gradient(circle, rgba(255,31,109,.13), rgba(255,31,109,0) 64%)}
  /* foto à direita, full-bleed */
  .photo{position:absolute;top:0;right:0;width:${PHOTO_W}px;height:630px;object-fit:cover;object-position:50% 22%}
  .photo-fade{position:absolute;top:0;right:${PHOTO_W - 1}px;width:120px;height:630px;
    background:linear-gradient(90deg, ${T.paper} 8%, rgba(250,246,239,0) 100%);z-index:2}
  /* bolha DM sobre a foto */
  .bub{position:absolute;z-index:3;left:${1200 - PHOTO_W - 44}px;bottom:74px;
    background:${T.brand};color:#fff;font-family:'Jakarta';font-weight:600;font-size:24px;line-height:1.26;
    padding:18px 24px;border-radius:22px;border-bottom-right-radius:7px;max-width:336px;
    box-shadow:0 26px 60px -20px rgba(255,31,109,.55)}
  .bub .auto{display:flex;align-items:center;gap:8px;margin-top:11px;font-size:13px;font-weight:700;
    text-transform:uppercase;letter-spacing:.09em;color:rgba(255,255,255,.92)}
  .bub .auto b{display:inline-grid;place-items:center;width:18px;height:18px;border-radius:50%;background:#fff;color:${T.brand};font-size:11px}
  /* coluna esquerda */
  .pad{position:absolute;inset:0;right:${PHOTO_W - 40}px;padding:54px 0 54px 66px;display:flex;flex-direction:column;z-index:4}
  .logo{display:flex;align-items:center}
  .body{margin-top:auto;margin-bottom:auto}
  .eyebrow{display:inline-block;font-family:'Jakarta';font-weight:700;font-size:21px;letter-spacing:0.15em;text-transform:uppercase;color:${T.brandInk};margin-bottom:22px}
  .head{font-family:'Bricolage';font-weight:800;font-size:86px;line-height:0.98;letter-spacing:-0.035em;color:${T.ink}}
  .hl{position:relative;white-space:nowrap;color:${T.brand}}
  .swash{position:absolute;left:0;right:-4px;bottom:-13px;width:104%;height:20px}
  .sub{font-family:'Jakarta';font-weight:500;font-size:29px;line-height:1.34;color:${T.inkSoft};margin-top:28px;max-width:600px;text-wrap:balance}
  .sub i{font-style:italic;color:${T.ink}}
  /* selos — chips brancos idênticos */
  .seals{display:flex;align-items:center;gap:18px}
  .seal-chip{display:inline-flex;align-items:center;justify-content:center;height:84px;padding:0 30px;background:#fff;
    border:1px solid rgba(20,16,14,.09);border-radius:18px;box-shadow:0 10px 26px -10px rgba(20,16,14,.20)}
  .seal-chip img{width:auto;display:block}
  .seal-chip .meta{height:54px}
  .seal-chip .tt{height:46px}
</style></head>
<body>
  <div class="og">
    <div class="glow"></div>
    ...
    <div class="bub">Te mandei tudo no direct<div class="auto"><b>✓</b> resposta automática</div></div>

    <div class="pad">
      <div class="logo">${WORDMARK}</div>
      <div class="body">
        <span class="eyebrow">Instagram + TikTok · API oficial</span>
        <h1 class="head">Eles comentam.<br>A gente <span class="hl">vende${SWASH}</span>.</h1>
        <p class="sub">Responde comentários, DMs e stories na hora — com a <i>sua</i> voz, pela API oficial da Meta.</p>
      </div>
      <div class="seals">
...
await pg.evaluate(async () => { await document.fonts.load("800 72px Bricolage"); await document.fonts.ready; });
const buf = await pg.screenshot({ type: 'jpeg', quality: 92, clip: { x: 0, y: 0, width: 1200, height: 630 } });
await browser.close();
writeFileSync(OUT, buf);
console.log(`✅ OG v2 salva em ${OUT} (${(buf.length/1024).toFixed(0)} KB)`);
