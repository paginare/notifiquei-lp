...
// Full wordmark SVG (from the HTML symbol)
const LOGO_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 290 50" width="580" height="100" fill="#FF1F6D">
...
async function svgToPng(svg) {
  ...
  await page.setViewport({ width: 580, height: 100 });
  await page.setContent(`<html><body style="margin:0;background:#1A1416;padding:20px">${svg}</body></html>`);
  const buf = await page.screenshot({ type: 'png', clip: { x: 0, y: 0, width: 580, height: 100 } });
  await browser.close();
  return buf;
}

async function generateOG(logoPngB64) {
  const prompt = `You are given the official logo of "Notifiquei", a Brazilian Instagram automation SaaS. Create a professional, premium Open Graph image (1200x630) for their website.

Design requirements:
- Background: very dark warm charcoal (#1A1416), clean and minimal
- Top center or left: show the exact "Notifiquei" wordmark from the reference image provided — reproduce it faithfully in hot pink/magenta (#FF1F6D)
- Below the logo: tagline "Você posta. A gente vende." in large, bold cream/off-white text (~36–42px)
- Secondary line below: "Automação de Instagram · API oficial Meta" in smaller muted gray text
- Right side decoration: 2–3 floating Instagram DM bubble icons (rounded rectangles with speech bubble tail) in pink/magenta, slightly blurred depth-of-field effect
- Bottom right: badge-style element "2.000+ contas ativas" with a small star icon
- Bottom left: "notifiquei.com.br" in muted gray small text
- Overall: modern, premium SaaS, clean grid, Brazilian digital brand
- NO gradients that look cheap, NO clip art, NO stock photo aesthetic
- Landscape 16:9, feels like a Stripe or Linear card`;

  const body = JSON.stringify({
    contents: [{
      parts: [
        { text: prompt },
        { inlineData: { mimeType: 'image/png', data: logoPngB64 } }
      ]
    }],
    generationConfig: {
  ...
  for (const base of ENDPOINTS) {
    ...
    try {
      ...
      if (!res.ok) { const t = await res.text(); console.log(`  HTTP ${res.status}:`, t.slice(0,200)); continue; }
...
console.log('1. Renderizando logo SVG → PNG via Puppeteer...');
const logoPng = await svgToPng(LOGO_SVG);
const logoPngB64 = logoPng.toString('base64');
writeFileSync('/tmp/notifiquei-logo-ref.png', logoPng);
console.log(`   Logo PNG: ${logoPng.length} bytes`);

console.log('2. Gerando OG image com Gemini...');
await generateOG(logoPngB64);
