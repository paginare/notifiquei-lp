...
6. Rodar `npm run check:blog`: a teia pilar/satélite de `docs/blog-clusters.md` tem que continuar sem pendência. Se o item mexeu em link interno, conferir aqui antes de seguir.
7. `npm install` e `npm run build`. O build tem que passar. Conferir `grep -c '"@type":"BlogPosting"' dist/blog/<slug>.html` (=1) em cada post alterado.
